namespace Snebur.Utilidade
{
    ...
    class HtmlEncode
    {
        ...
        public Encode(str: string): string
        {
            ...
            while (i < strLength)
            {
                const charInfo = this.IndicesCaracteres[str.charCodeAt(i)];
                if (charInfo)
                {
                    let alpha = charInfo[str.charCodeAt(i + 1)];
                    if (alpha)
                    {
                        i++;
                    } else
