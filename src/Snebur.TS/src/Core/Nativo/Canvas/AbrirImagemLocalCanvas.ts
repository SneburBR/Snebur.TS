namespace Snebur.Imagens
{
    export class AbrirImagemLocalCanvas extends BaseAbrirImagemLocalCanvas 
    {
        ...
        private async AbrirImagemAsync(imagemAtual: HTMLImageElement)
        {
            ...
            let [canvas, motor] = await super.RetornarCanvasAsync(imagemAtual, dimensaoApresentacao);
            ...
            for (const tamanhoImagem of tamanhos)
            {
                ...
                if (tamanhoImagem !== EnumTamanhoImagem.Grande)
                {
                    ...
                    [canvas, motor] = await super.RetornarCanvasAsync(canvas, dimensaoApresentacao);
                ...
                if (tamanhoImagem === d.EnumTamanhoImagem.Grande)
                {
                    ...
                    const nomeArquivoOriginal = this.ArquivoLocal.Name;
                    const opcoes: IOpcoesCanvas = {
                        Motor: motor,
                        NomeArquivoOrigem: nomeArquivoOriginal,
                        Qualidade: ImagemUtil.QUALIDADE_APRESENTACAO_CANVAS,
                        Altura: dimensaoApresentacao.Altura,
                        Largura: dimensaoApresentacao.Largura,
                        MimeType: mimeType
                    };

                    const resultado = {
                        LarguraImagemOrigem: dimensaoImagemOriginal.Largura,
                        AlturaImagemOrigem: dimensaoImagemOriginal.Altura,
                        Blob: blob,
                        LarguraImagem: dimensaoApresentacao.Largura,
                        AlturaImagem: dimensaoApresentacao.Altura,
