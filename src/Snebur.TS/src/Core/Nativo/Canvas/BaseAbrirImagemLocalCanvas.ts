namespace Snebur.Imagens
{
    export abstract class BaseAbrirImagemLocalCanvas implements IDisposable
    {
        ...
        private _errorPicaCount: number = 0;
        ...
        protected RetornarImagemData(
            imagem: HTMLImageElement,
            dimensao: IDimensao): [HTMLCanvasElement, ImageData]
        {
        ...
        protected async RetornarCanvasAsync(
            imagem: HTMLImageElement | HTMLCanvasElement,
            dimensao: IDimensao):
            Promise<[HTMLCanvasElement, EnumMotorProcessamentoImagem]>
        {
            try
            {
                if (window.MOTOR_PROCESSAMENTO_IMAGEM !== EnumMotorProcessamentoImagem.Canvas)
                {
            ...
            {
                this._errorPicaCount++;
                console.error(`PicaJS - Erro ao redimensionar imagem. Erro: ${erro}`);

                if (this._errorPicaCount > 3)
                {
                    window.SetMotorProcessamentoImagem(EnumMotorProcessamentoImagem.Canvas);
                }
            }
            return this.RetornarCanvas_Normal(imagem, dimensao);
        }

        protected async RetornarCanvasPicaAsync(
            imagem: HTMLImageElement | HTMLCanvasElement,
            dimensao: IDimensao)
            : Promise<[HTMLCanvasElement, EnumMotorProcessamentoImagem]>
        {
            ...
            {
                throw new Error("Falha no redimensionamento do Pica, Arquivo: " + this.ArquivoLocal.name);
            }

            return [resultado, EnumMotorProcessamentoImagem.PicaJS];
        }

        protected async RetornarCanvas_Normal(
            imagem: HTMLImageElement | HTMLCanvasElement, dimensao: IDimensao)
            : Promise<[HTMLCanvasElement, EnumMotorProcessamentoImagem]>
        {
            const canvas = document.createElement("canvas");
            canvas.width = dimensao.Largura;
            ...
            const contexto = canvas.getContext2DRequired();
            contexto.drawImage(imagem, 0, 0, dimensao.Largura, dimensao.Altura);
            return [canvas, EnumMotorProcessamentoImagem.Canvas];
            /*return canvas;*/
        }
