# `<sn-expandir-conteudo>` — Expander content

**Type:** Form control (`sn-`) | **Class:** `ExpandirConteudo` | **Extends:** `BaseControle` | **Rendered as:** `HTMLDivElement`

Collapsible body of a `sn-expandir` section.
...
`ExpandirConteudo` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
