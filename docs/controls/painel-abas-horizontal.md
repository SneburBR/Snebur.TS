# `<sn-painel-abas-horizontal>` — Tab panel (horizontal)

**Type:** Form control (`sn-`) | **Class:** `PainelAbasHorizontal` | **Extends:** `BaseControleApresentacaoFormulario` | **Rendered as:** `HTMLDivElement`
...
Extracted from the Sigi frontend (12 usages in 12 files).

```html
<sn-painel-abas-horizontal
    sn-nome="PainelAbas"
    ap-tipo-painel-aba="MaterialDesign"
    sn-is-abas-cheia="false"
    sn-is-manter-cache="true"
    ap-tipo-animacao="Deslizante"
    sn-bind-data-source="{{.}}"
    sn-funcao-is-pode-navegar-aba="this.IsPodeNavegarAsync"
...
<sn-painel-abas-horizontal
    ...
    ap-tipo-painel-aba="MaterialDesign"
    sn-is-abas-cheia="false"
    sn-is-manter-cache="false"
    ap-tipo-animacao="Deslizante"
    sn-pagina-inicial="PaginaPromocaoCupons"
    sn-bind-data-source="{{.}}"
    sn-is-propagar-bind-data-source="true">
    ...
</sn-painel-abas-horizontal>
...
*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Promocao/Codigos/JanelaVisualizarCuponsPromocao.shtml`*
...
<sn-painel-abas-horizontal
        ...
        <sn-aba
            sn-navegar="PaginaCadastroPromocao">
            Cadastro
        </sn-aba>

        <sn-aba sn-navegar="PaginaCadastroPromocaoEntregas">
            Entregas
        </sn-aba>

        <sn-aba sn-navegar="PaginaCadastroPromocaoPagamentos">
            Pagamentos
        </sn-aba>
    </sn-painel-abas-horizontal>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Promocao/Cadastro/Editar/JanelaVisualizarPromocao.shtml`*
...
|-----------|------|-------------|-------------|
| `sn-carregar-aba-inicial` | `Boolean` | `PainelAbasHorizontal` | Loads the initial tab content eagerly. |
| `sn-funcao-is-pode-navegar-aba` | `String` | `PainelAbasHorizontal` | Codebehind predicate that can veto switching tabs (unsaved changes…). |
| `sn-identificador-navegador` | `String` | `PainelAbasHorizontal` | Name of the target navigator (for buttons/links that navigate). |
| `sn-is-abas-cheia` | `Boolean` | `PainelAbasHorizontal` | Stretches tabs to fill the full strip width. |
| `sn-is-manter-cache` | `Boolean` | `PainelAbasHorizontal` | Keeps navigated pages cached (state preserved when returning). |
| `sn-pagina-inicial` | `String` | `PainelAbasHorizontal` | Initial page/control shown by the navigator. |
| `sn-parametros` | `String` | `PainelAbasHorizontal` | Parameters passed to the hosted control constructor (comma-separated / bind paths). |
...
| `ap-tipo-painel-aba` | [`EnumTipoPainelAba`](../ENUMS.md#enumtipopainelaba) | Visual style of the tab strip: Padrao or MaterialDesign. |
...
`PainelAbasHorizontal` → `BaseControleApresentacaoFormulario` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **12** usages across **12** `.shtml` files in the Sigi frontend.
