# `<sn-colunas>` — Grid column collection

**Type:** Form control (`sn-`) | **Class:** `TemplateColunasColecao` | **Extends:** `BaseTemplate` | **Rendered as:** `HTMLDivElement`

Container declaring the columns of a `sn-data-lista`.

## Real-world examples

Extracted from the Sigi frontend (198 usages in 187 files).

```html
<sn-colunas sn-click="SelecionarLinha_Click">
    ...
</sn-colunas>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Seguranca/PaginaSessoes.shtml`*
...
```html
<sn-colunas>
        <sn-coluna-texto sn-bind="{{DescricaoCompleta}}" sn-rotulo="Produtos"></sn-coluna-texto>
        <sn-coluna-texto sn-rotulo="Quant." sn-bind="{{Quantidade}}" class="sn-centro"></sn-coluna-texto>
        <sn-coluna-texto sn-rotulo="Unit." sn-bind="{{ValorUnitario}}, Formatar=Moeda" class="sn-direita" ap-visibilidade--celular="Oculto"></sn-coluna-texto>
        <sn-coluna-personalizada sn-rotulo="Total" ap-largura="120px" class="sn-direita">
            <ap-painel ap-tipo-painel="PilhaHorizontal">
                <ap-bloco sn-visibilidade="{{IsPromocao}}">
                    <ap-icone ap-icone="LocalOffer" ap-cor-texto="Cinza" ap-margem="0px 0px 8px 0px"></ap-icone>
                </ap-bloco>
                <ap-bloco>
                    <ap-texto sn-bind="{{ValorTotal}}, Formatar=Moeda"></ap-texto>
                </ap-bloco>
            </ap-painel>
...
|-------|--------|------------|
| `sn-click` | `TemplateColunasColecao` | Fires when the element is clicked/tapped. Value = codebehind method name. |
...
**`sn-click="SelecionarLinha_Click"`** — `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Seguranca/PaginaSessoes.shtml.ts`:

```ts
private SelecionarLinha_Click(provedor: ui.BaseUIElemento, e: ui.UIEventArgs)
{
...
`TemplateColunasColecao` → `BaseTemplate` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **198** usages across **187** `.shtml` files in the Sigi frontend.
