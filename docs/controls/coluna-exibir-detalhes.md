# `<sn-coluna-exibir-detalhes>` — Details toggle column

**Type:** Form control (`sn-`) | **Class:** `TemplateColunaExibirDetalhes` | **Extends:** `TemplateColuna` | **Rendered as:** `HTMLDivElement`

Grid column with the expander button that shows the row `sn-linha-detalhes`.

## Real-world examples

Extracted from the Sigi frontend (18 usages in 18 files).

```html
<sn-coluna-exibir-detalhes
        ap-largura="26px" 
        sn-visibilidade="{{IsMostrrarExpandirDetalhes}}, Origem=this">
    </sn-coluna-exibir-detalhes>
```

*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Pedido/ControleDadosRevelacao.shtml`*

```html
<sn-coluna-exibir-detalhes ap-largura="25px"></sn-coluna-exibir-detalhes>
```

*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Pedido/paginapedidos.shtml`*

```html
<sn-coluna-exibir-detalhes ap-largura="50px">
    </sn-coluna-exibir-detalhes>
```

*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Pedido/ControleDadosEstojos.shtml`*
...
`TemplateColunaExibirDetalhes` → `TemplateColuna` → `BaseTemplate` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **18** usages across **18** `.shtml` files in the Sigi frontend.
