# `<sn-expandir-titulo>` — Expander header

**Type:** Form control (`sn-`) | **Class:** `ExpandirTitulo` | **Extends:** `BaseControle` | **Rendered as:** `HTMLDivElement`

Clickable header of a `sn-expandir` section.
...
`ExpandirTitulo` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
