# `<sn-painel-lista-selecao>` — Selection list panel

**Type:** Form control (`sn-`) | **Class:** `PainelListaSelecao` | **Extends:** `PainelLista` | **Rendered as:** `HTMLDivElement`
...
<sn-painel-lista-selecao
        ...
        sn-bind-itens-selecionado="{{TipoGravacaoEstojoSuportado.LinhasRevestimentoSuportado}}"
        sn-bind-lista="{{LinhasRevestimento}}"
        sn-item-selecionado-alterado="ControleBairros_ItemSelecionadoAlterado">

        <sn-bloco-template>
            <sn-checkbox sn-rotulo="{{Nome}}" ></sn-checkbox>
        </sn-bloco-template>

        <sn-bloco-template-selecionado>
            <sn-checkbox sn-rotulo="{{Nome}}" class="sn-checkbox-selecionado"  ></sn-checkbox>
        </sn-bloco-template-selecionado>

    </sn-painel-lista-selecao>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Encadernacao/Relacionamentos/Estojo/04_Gravacao/JanelaTipoGravacaoEstojoLinhasRevestimentoSuportado.shtml`*

```html
<sn-painel-lista-selecao
        ap-tipo-painel="PilhaVertical"
        sn-bind-itens-selecionado="{{TipoGravacaoCapaSuportado.LinhasRevestimentoSuportado}}"
        sn-bind-lista="{{LinhasRevestimento}}"
        sn-item-selecionado-alterado="ControleBairros_ItemSelecionadoAlterado">
        ...
        <sn-bloco-template-selecionado>
            <sn-checkbox sn-rotulo="{{Nome}}" 
                class="sn-checkbox-selecionado"></sn-checkbox>
        </sn-bloco-template-selecionado>

    </sn-painel-lista-selecao>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Encadernacao/Relacionamentos/Album/Capa/Relacionamentos/04_Gravacao/JanelaTipoGravacaoCapaLinhasRevestimentoSuportado.shtml`*
...
**`sn-item-selecionado-alterado="ControleBairros_ItemSelecionadoAlterado"`** — `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Encadernacao/Relacionamentos/Estojo/04_Gravacao/JanelaTipoGravacaoEstojoLinhasRevestimentoSuportado.shtml.ts`:

```ts
private ControleBairros_ItemSelecionadoAlterado(provedor: ui.BaseUIElemento, e: ui.ItemSelecionadoEventArgs)
{
    if (!e.IsSelecionado && this.TipoGravacaoEstojoSuportado.IsTodasLinhaRevestimentoSuportada)
    {
        this.TipoGravacaoEstojoSuportado.IsTodasLinhaRevestimentoSuportada = false;
    }
}
```
...
`PainelListaSelecao` → `PainelLista` → `BaseControleLista` → `BaseControleApresentacaoFormulario` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
