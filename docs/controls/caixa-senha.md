# `<sn-caixa-senha>` — Password box

**Type:** Form control (`sn-`) | **Class:** `CaixaSenha` | **Extends:** `BaseCaixaTexto` | **Rendered as:** `HTMLDivElement`

Password input with show/hide toggle. `sn-is-senha-md5` hashes the value before binding (legacy flows).

## Real-world examples

Extracted from the Sigi frontend (31 usages in 17 files).

```html
<sn-caixa-senha
        sn-nome="TxtSenha"
        sn-rotulo="Informe sua senha"
        sn-bind="{{Senha}}"
        sn-is-senha-md5="false"
        sn-mensagem-validacao-flutuante="true"
        sn-enter="TxtSenha_Enter"
        ap-altura="70px"
        ap-largura="100%">
    </sn-caixa-senha>
```
...
<sn-caixa-senha
        ...
        sn-bind="{{Senha}}"
        sn-is-senha-md5="false"
        sn-manter-espaco-mensagem-validacao="true"
        sn-mensagem-validacao-flutuante="false">
    </sn-caixa-senha>
```

*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Soliticacao/Login/RecuperarCadastro/PaginaRecuperarCadastro.shtml`*

```html
<sn-caixa-senha
        sn-nome="TxtNovaSenha"
        sn-rotulo="Nova senha"
        sn-bind="{{NovaSenha}},Origem=this"
        sn-is-senha-md5="false"
        ap-largura="100%"
        ap-altura="50px">
    </sn-caixa-senha>
...
*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Soliticacao/Login/PaginaConfirmacaoRecuperacaoSenha.shtml`*
...
|-----------|------|-------------|-------------|
| `sn-is-senha-md5` | `Boolean` | `CaixaSenha` | Hashes the typed password with MD5 before binding (legacy authentication flows). |
...
**`sn-enter="TxtSenha_Enter"`** — `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Soliticacao/Login/PaginaLogin.shtml.ts`:

```ts
private async TxtSenha_Enter(provedor: ui.BaseUIElemento, e: ui.UIEventArgs)
{
    if (String.IsNullOrWhiteSpace(this.LoginViewModel.Senha))
    {
        await this.ValidarFormularioAsync();
        return;
    }

    if (String.IsNullOrWhiteSpace(this.LoginViewModel.Email))
    {
        this.TxtEmail.Focus();
    }
    else
...
|------|-------------|-------------|
| `sn-bind-senha` | `CaixaSenha` | Binds a password value. |
| `sn-bind-texto` | `CaixaSenha` | One-way bind that renders the value as the element text (also `{{path}}` in content). |
...
`CaixaSenha` → `BaseCaixaTexto` → `BaseControleFormulario` → `ControleRotulo` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **31** usages across **17** `.shtml` files in the Sigi frontend.
