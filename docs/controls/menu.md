...
**Type:** Form control (`sn-`) | **Class:** `Menu` | **Extends:** `BaseControle` | **Rendered as:** `HTMLDivElement`
...
```html
<sn-menu sn-nome="MenuNavegacaoLateral">
    ...
</sn-menu>
```
...
*Source: `src/frontend/Zyoncore.Sigi.ZyonAdmin/Apresentacao/MenuLateral.shtml`*
...
`Menu` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
