# `<sn-coluna-texto>` — Text column

**Type:** Form control (`sn-`) | **Class:** `TemplateColunaTexto` | **Extends:** `TemplateColuna` | **Rendered as:** `HTMLDivElement`

Grid column rendering a bound text value (`sn-bind-texto` / `{{path}}`), with optional formatting (`sn-formatar`).

## Real-world examples

Extracted from the Sigi frontend (213 usages in 61 files).

```html
<sn-coluna-texto sn-bind="{{ValorUnitario}}"
        sn-rotulo="Unit."
        ap-largura="100px"
        sn-formatar="Moeda"
        class="sn-direita" ap-visibilidade--celular="Oculto">
    </sn-coluna-texto>
```

*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Pedido/ControleDadosProduto.shtml`*

```html
<sn-coluna-texto sn-rotulo="Itens"
                     sn-bind="{{Itens}}"
                     class="sn-centro"
                     ap-largura="50px"
...
```html
<sn-coluna-texto sn-rotulo="Unit." sn-bind="{{ValorUnitario}}, Formatar=Moeda" class="sn-direita"
                     ap-visibilidade--celular="Oculto">
    </sn-coluna-texto>
```
...
|-----------|------|-------------|-------------|
| `sn-formatar` | [`EnumFormatacao`](../ENUMS.md#enumformatacao) | `TemplateColunaTexto` | Formatting applied to the bound value for display (currency, date, CPF/CNPJ, percentage…). |
...
|------|-------------|-------------|
| `sn-bind` | `TemplateColunaTexto` | Two-way bind of the control **value** to a property path of the data source (e.g. `Cliente.Nome`). Validation rules of the bound domain property are applied automatically. |
| `sn-bind-texto` | `TemplateColunaTexto` | One-way bind that renders the value as the element text (also `{{path}}` in content). |
...
`TemplateColunaTexto` → `TemplateColuna` → `BaseTemplate` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **213** usages across **61** `.shtml` files in the Sigi frontend.
