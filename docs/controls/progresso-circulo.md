# `<sn-progresso-circulo>` — Circular progress

**Type:** Form control (`sn-`) | **Class:** `ProgressoCirculo` | **Extends:** `BaseControle` | **Rendered as:** `HTMLDivElement`

Circular/spinner variant of the progress indicator.
...
<sn-progresso-circulo
        ...
        ap-cor-borda="Amarelo"
        sn-bind-progresso="{{ProgressoMontagem}}">
    </sn-progresso-circulo>
```
...
`ProgressoCirculo` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
