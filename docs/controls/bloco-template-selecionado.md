# `<sn-bloco-template-selecionado>` — Selected block template

**Type:** Form control (`sn-`) | **Class:** `BlocoTemplateSelecionado` | **Extends:** `BaseBlocoTemplate` | **Rendered as:** `HTMLDivElement`

Template for the selected item of a `sn-painel-lista-selecao`.

## Real-world examples

Extracted from the Sigi frontend (8 usages in 4 files).

```html
<sn-bloco-template-selecionado>
        <ap-texto sn-bind="{{DescricaoSelecionado}}"
            ap-altura-linha="28px">
        </ap-texto>
    </sn-bloco-template-selecionado>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Pedidos/Controles/ControleFiltrosPedido.shtml`*
...
`BlocoTemplateSelecionado` → `BaseBlocoTemplate` → `BaseTemplate` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **8** usages across **4** `.shtml` files in the Sigi frontend.
