# `<sn-data-lista-ordenacao>` — Data grid with row reordering

**Type:** Form control (`sn-`) | **Class:** `DataListaOrdenacao` | **Extends:** `DataLista` | **Rendered as:** `HTMLDivElement`

Data grid variant whose rows can be drag-reordered.

## Real-world examples

Extracted from the Sigi frontend (52 usages in 52 files).

```html
<sn-data-lista-ordenacao
    sn-entidade-ordenacao="Promocao"
    sn-is-ativar-ordenacao="this.IsAtivarOrdenacao"
    sn-nome="DataLista"
    ap-margem="0px 0px 0px 16px"
    sn-bind-lista="{{Itens}}, Origem=this">
    ...
</sn-data-lista-ordenacao>
```
...
<sn-data-lista-ordenacao
    sn-nome="DataLista"
    sn-tipo="fotoAlbumE.LinhaAlbum"
    ap-margem="0px 0px 0px 16px"
    sn-bind-lista="{{Itens}}, Origem=this"
    sn-entidade-ordenacao="LinhaAlbum">
    ...
</sn-data-lista-ordenacao>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Encadernacao/Album/LinhaAlbum/PaginaLinhasAlbum.shtml`*

```html
<sn-data-lista-ordenacao
    sn-nome="DataLista"
    ap-margem="0px 0px 0px 16px"
    sn-bind-lista="{{Entidades}}, Origem=this"
     class="sn-data-lista-ocultar-cabecalho" >
    ...
</sn-data-lista-ordenacao>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Sigi/FotoProduto/Cadastro/PaginaCadastroFotoProdutoAtributos.shtml`*
...
|-----------|------|-------------|-------------|
| `sn-entidade-ordenacao` | `String` | `DataListaOrdenacao` | Entity/property that stores the order index. |
| `sn-is-ativar-ordenacao` | `Boolean` | `DataListaOrdenacao` | Enables column sorting on the data grid. |
...
| `sn-desativar-efeito-hover` | `Boolean` | `DataLista` *(inherited)* | Disables the row hover effect. |
...
| `sn-is-marcar-item` | `Boolean` | `DataLista` *(inherited)* | Debug: highlights the item under the cursor. |
| `sn-is-marcar-linha` | `Boolean` | `DataLista` *(inherited)* | Debug: highlights the row under the cursor. |
...
|-------|--------|------------|
| `sn-linha-click` | `DataLista` *(inherited)* | Fires when a grid row is clicked. |
...
`DataListaOrdenacao` → `DataLista` → `BaseControleLista` → `BaseControleApresentacaoFormulario` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **52** usages across **52** `.shtml` files in the Sigi frontend.
