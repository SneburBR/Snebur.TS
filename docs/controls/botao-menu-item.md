# `<sn-botao-menu-item>` — Menu item button

**Type:** Form control (`sn-`) | **Class:** `BotaoMenuItem` | **Extends:** `ControleRotulo` | **Rendered as:** `HTMLDivElement`

An item inside a `sn-botao-menu` floating menu — icon + label, fires `sn-click` when chosen.

## Real-world examples

Extracted from the Sigi frontend (28 usages in 7 files).

```html
<sn-botao-menu-item sn-click="BtnTabelaPrecoAlbum_Click"
          sn-rotulo="{{DescricaoPrecoAlbumLaminaAdicional}}, Origem=this" >
        Álbum 
    </sn-botao-menu-item>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Encadernacao/Relacionamentos/Album/PaginaEdicaoLinhaAlbum.shtml`*

```html
<sn-botao-menu-item
        sn-click="BtnTabelaPrecoEstojo_Click"
        sn-visibilidade="{{LinhaAlbum.IsPossuiEstojo }}, Origem=this">
        Estojo
    </sn-botao-menu-item>
```
...
<sn-botao-menu-item
        sn-visibilidade="{{LinhaAlbum.IsPossuiEstojo}}, Origem=this"
        sn-click="BtnTabelaPrecoAvancaoEstojo_Click">
        Estojo
    </sn-botao-menu-item>
```
...
**`sn-click="BtnRandom_Click"`** — `src/frontend/Zyoncore.Sigi.ZyonAdmin/Apresentacao/Cadastros/Gabarito/Relatorio/JanelaRelatorioOrientacoesGabarito.shtml.ts`:

```ts
public async BtnRandom_Click(botao: ui.Botao, e: ui.UIEventArgs)
{
    const faixas = parseInt(e.Parametros.Item("faixas"));
    const orientacoes = GrupoOrientacaoUtil.RetornarOrientacoesAleatoria(this.NumeroFotos);
    const grupoOrientacao = new GrupoOrientacao(orientacoes, GrupoOrientacaoUtil.RetornarChave(orientacoes));
    await this.GerarGabaritoAsync(faixas, grupoOrientacao);
}
```
...
`BotaoMenuItem` → `ControleRotulo` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **28** usages across **7** `.shtml` files in the Sigi frontend.
