# `<sn-caixa-prazo-tempo>` — Time-span box

**Type:** Form control (`sn-`) | **Class:** `CaixaPrazoTempo` | **Extends:** `ControleRotulo` | **Rendered as:** `HTMLDivElement`

Input for a time span / deadline (complex domain type `PrazoTempo`).
...
`CaixaPrazoTempo` → `ControleRotulo` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
