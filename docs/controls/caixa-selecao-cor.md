# `<sn-caixa-selecao-cor>` — Color picker

**Type:** Form control (`sn-`) | **Class:** `CaixaSelecaoCor` | **Extends:** `BaseControle` | **Rendered as:** `HTMLDivElement`

Input for choosing one of the framework palette colors. Bind with `sn-bind-selecao-cor`.

## Real-world examples

Extracted from the Sigi frontend (13 usages in 8 files).

```html
<sn-caixa-selecao-cor ap-margem="16px 0px 0px 0px" sn-valor-alterado="CxSelecaoCorGravacao_ValorAlterado" sn-nome="CxSelecaoCorGravacao" sn-bind="{{CorSelecionada}}, Origem=this"></sn-caixa-selecao-cor>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Encadernacao/Compartilhado/TipoGravacao/Texto/ControleCadastroCorTipoGravacaoTexto.shtml`*

```html
<sn-caixa-selecao-cor
        sn-nome="CaixaSelecaoCor"
        sn-flutuante="false"
        sn-bind="{{Borda.Cor}}, Origem=this"[
        sn-valor-alterado="CaixaSelecaoCor_ValorAlterado" >
    </sn-caixa-selecao-cor>
```
...
<sn-caixa-selecao-cor
        ...
        sn-nome="CaixaCorContorno"
        ap-icone="FontDownload"
        ap-icone-categoria="Outlined"
        sn-valor-alterado="CaixaSelecaoCorContorno_ValorAlterado"
        sn-legenda="Cor do contorno">
        Contorno
    </sn-caixa-selecao-cor>
```

*Source: `src/frontend/Zyoncore.Sigi.FotoAlbum.TS/Apresentacao/Montagem/FerramentasFlutuante/ControleFerramentaTexto.shtml`*
...
|-----------|------|-------------|-------------|
| `sn-flutuante` | `Boolean` | `CaixaSelecaoCor` | Renders the control floating over the page content. |
...
**`sn-valor-alterado="CxSelecaoCorGravacao_ValorAlterado"`** — `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Encadernacao/Compartilhado/TipoGravacao/Texto/ControleCadastroCorTipoGravacaoTexto.shtml.ts`:

```ts
private CxSelecaoCorGravacao_ValorAlterado(provedor: ui.BaseUIElemento, e: ui.UIValorAlteradoEventArgs)
{            this.TxtMensagemErro.OcultarElemento();
}
```
...
`CaixaSelecaoCor` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **13** usages across **8** `.shtml` files in the Sigi frontend.
