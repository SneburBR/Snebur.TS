# `<sn-menu-sanfona>` — Accordion menu

**Type:** Form control (`sn-`) | **Class:** `MenuSanfona` | **Extends:** `BaseMenu` | **Rendered as:** `HTMLDivElement`

Menu with collapsible (accordion) sections.

## Real-world examples

Extracted from the Sigi frontend (19 usages in 2 files).
...
<sn-menu-sanfona
        ...
        sn-visibilidade="{{IsMostrarFotografo}}, Origem=this">

        <sn-menu-item
            sn-rotulo="Eventos"
            sn-navegar="PaginaEventos"
            sn-parametros="IsPaginaBase=True"
            sigi-permissao="PermissaoEstudioEventos.Eventos">
        </sn-menu-item>
    </sn-menu-sanfona>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/MenuLateral.shtml`*

```html
<sn-menu-sanfona sn-nome="MenuItemMinhaConta"
                     sn-rotulo="Minha Conta"
                     ap-icone="Person"
                     ap-icone-categoria="Outlined"
                     sn-parametros="IsPaginaBase=True"
                     sn-tags="senha, sessoes,usuarios">

        <sn-menu-item sn-rotulo="Sessões"
                      sn-navegar="PaginaMinhaConta"
                      sn-parametros="IsPaginaBase=True">
        </sn-menu-item>

        <sn-menu-item sn-rotulo="Usuários"
                      sn-navegar="PaginaUsuarios"
                      sn-parametros="IsPaginaBase=True"
                      sigi-permissao="PermissaoMinhaConta.Usuarios">
        </sn-menu-item>
...
```html
<sn-menu-sanfona sn-rotulo="Revelação"
    ap-icone="Image"
    ap-icone-categoria="Outlined">
    ...
</sn-menu-sanfona>
...
| `sn-rotulo` | `String` | `BaseMenu` *(inherited)* | Label text of the control (floating label on inputs). |
| `sn-tags` | `String` | `BaseMenu` *(inherited)* | Free-form tags attached to the control (comma separated) — queried from code. |
...
`MenuSanfona` → `BaseMenu` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **19** usages across **2** `.shtml` files in the Sigi frontend.
