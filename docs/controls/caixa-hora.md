# `<sn-caixa-hora>` — Time box

**Type:** Form control (`sn-`) | **Class:** `CaixaHora` | **Extends:** `BaseCaixaTexto` | **Rendered as:** `HTMLDivElement`

Time-of-day input (`HH:mm`).

## Real-world examples

Extracted from the Sigi frontend (7 usages in 6 files).

```html
<sn-caixa-hora sn-bind="{{PrazoProducao.Tempo}}" sn-is-rotulo-vazio="true" ap-largura="70px"
        sn-visibilidade="{{PrazoProducao.TipoPrazo}}, Converter=this.RetornarVisibilidadePrazoHoras"
        sn-bind-css-classe="{{PrazoProducao.Tempo}}, Converter=this.RetornarClassCssPrazoProducao"
        sn-opcao-bind-css-classe="Alterar"
        sn-opcao-bind-css-classe-prefixo-alterar="prazo-producao--">
    </sn-caixa-hora>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Encadernacao/Relacionamentos/Compartilhado/BaseJanelaConfiguracaoPesoPrazoProducao.shtml`*
...
<sn-caixa-hora
        ...
        ap-largura="100%"
        sn-bind="{{HoraEntregaAdiconal}}, Origem=this"
        sn-desabilitar="{{IsPossuiPrazoHoras === false}}, Origem=this">
    </sn-caixa-hora>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Sigi/Revelacao/JanelaCadastroTamanhoRevelacao.shtml`*

```html
<sn-caixa-hora
        sn-bind="{{PrazoEmHoras}}"
        sn-is-rotulo-vazio="true"
        sn-desabilitar="{{IsPrazoEmHoras == false}},Padrao=false"
        ap-largura="150px">
    </sn-caixa-hora>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Cobranca/EntregaPagamento/Entrega/PaginaCadastroEntrega.shtml`*
...
|-----------|------|-------------|-------------|
| `sn-is-hora-dia` | `String` | `CaixaHora` | Restricts the time to day hours. |
| `sn-mascara` | `String` | `CaixaHora` | Input mask (e.g. `(00) 0000-0000`). Masked input keeps the raw value in the bind. |
...
|------|-------------|-------------|
| `sn-bind-hora` | `CaixaHora` | Binds a time value rendered as text. |
...
`CaixaHora` → `BaseCaixaTexto` → `BaseControleFormulario` → `ControleRotulo` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **7** usages across **6** `.shtml` files in the Sigi frontend.
