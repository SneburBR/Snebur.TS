# `<sn-botao-icone-descricao>` — Icon + description button

**Type:** Form control (`sn-`) | **Class:** `BotaoIconeDescricao` | **Extends:** `ControleRotulo` | **Rendered as:** `HTMLButtonElement`

Button variant showing an icon with a text description below/next to it (used for dashboards and toolbars).

## Real-world examples

Extracted from the Sigi frontend (53 usages in 6 files).

```html
<sn-botao-icone-descricao
        sn-nome="BtnFiltros"
        ap-margem="0px 8px"
        ap-margem--notebook="0px 6px"
        ap-icone="PhotoFilter"
        ap-tamanho-icone="Medio"
        ap-tipografia="Descricao"
        ap-largura="100%"
        ap-rotulo="Filtros"
        sn-click="BtnFiltros_Click"
        ap-visibilidade="Oculto">
    </sn-botao-icone-descricao>
```

*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Revelacao/Moderno/RevelacaoConfiguracaoLateralModerno.shtml`*

```html
<sn-botao-icone-descricao
        ap-margem-interna="4px 8px 0px 0px"
        ap-margem="0px 8px"
        ap-icone="ShoppingCart"
        ap-tamanho-icone="Medio"
        ap-tipografia="Descricao"
        ap-largura="100%"
        ap-altura="80px"

        ap-rotulo="Adicionar ao carrinho"
        ap-cor-fundo="SistemaPrincipal"
        ap-cor-texto="Branca"
        class="btn-carrinho"
        sn-click="BtnAdicionarCarrinho_Click">
    </sn-botao-icone-descricao>
```
...
```html
<sn-botao-icone-descricao
        ap-margem="0px 25px 0px 0px"
        sn-nome="BtnFundosDireita"
        sn-click="BtnFundos_Click"
        sn-rotulo="Fundos"
        ap-icone="BlurOn"
        ap-tamanho-icone="Medio"
        ap-tipografia="Descricao"
        sn-visibilidade="{{IsMostrarBotaoFundos}}, Origem=this"
        ap-largura="100%">
    </sn-botao-icone-descricao>
```
...
**`sn-click="BtnSelecionarArquivos_Click"`** — `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Revelacao/Moderno/RevelacaoConfiguracaoLateralModerno.shtml.ts`:

```ts
private BtnSelecionarArquivos_Click(provedor: ui.BaseUIElemento, e: ui.UIEventArgs)
{
    this.PaginaRevelacao.AbrirCaixaSelecaoImagensAsync();
}
```

**`sn-selecionar-arquivos="BtnSelecionarArquivos_Click"`** — `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Revelacao/Celular/ControleMenuRodapeCelular.shtml.ts`:
...
`BotaoIconeDescricao` → `ControleRotulo` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **53** usages across **6** `.shtml` files in the Sigi frontend.
