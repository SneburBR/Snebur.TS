# `<sn-item-template-separador>` — Item separator template

**Type:** Form control (`sn-`) | **Class:** `SeparadorTemplate` | **Extends:** `BaseTemplate` | **Rendered as:** `HTMLDivElement`

Optional template rendered between list items (separator row).
...
`SeparadorTemplate` → `BaseTemplate` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
