# `<sn-proxima-pagina>` — Next-page loader

**Type:** Form control (`sn-`) | **Class:** `ControleProximaPagina` | **Extends:** `BaseControlePaginacao` | **Rendered as:** `HTMLDivElement`
...
|-----------|------|-------------|-------------|
| `sn-registros-por-pagina` | `String` | `ControleProximaPagina` | Page size (records per page). |
...
`ControleProximaPagina` → `BaseControlePaginacao` → `ControleUsuario` → `ControleApresentacao` → `BaseControleApresentacaoFormulario` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
