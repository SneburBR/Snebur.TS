# `<sn-caixa-data>` — Date box

**Type:** Form control (`sn-`) | **Class:** `CaixaData` | **Extends:** `BaseCaixaTexto` | **Rendered as:** `HTMLDivElement`

Date input with calendar assistance. `sn-tipo-data` selects the accepted range semantics (birth date, past, future…).

## Real-world examples

Extracted from the Sigi frontend (16 usages in 8 files).
...
<sn-caixa-data
        ...
        sn-marca-dagua="Opcional"
        sn-enter="BtnFim_Enter"
        sn-bind="{{Fim}}, Origem=this">
    </sn-caixa-data>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Pedidos/FiltroPedido/JanelaFiltroPedidoData.shtml`*
...
<sn-caixa-data
        ...
        sn-tipo-entrada="Numero"
        ap-tipo-caixa="Delineada"
        sn-enter="BtnInicio_Enter"
        sn-bind="{{Inicio}}, Origem=this">
    </sn-caixa-data>
```
...
|-----------|------|-------------|-------------|
| `sn-mascara` | `String` | `CaixaData` | Input mask (e.g. `(00) 0000-0000`). Masked input keeps the raw value in the bind. |
| `sn-tipo-data` | [`EnumTipoData`](../ENUMS.md#enumtipodata) | `CaixaData` | Semantic date range accepted (birth date, past only, near future…). Drives validation and the calendar UI. |
...
### Values for `sn-tipo-data` (`EnumTipoData`)
...
**`sn-enter="BtnInicio_Enter"`** — `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Pedidos/FiltroPedido/JanelaFiltroPedidoData.shtml.ts`:

```ts
public BtnInicio_Enter(caixaTexto: ui.CaixaTexto, e: ui.UIEventArgs)
{
    this.Filtrar();
}
```
...
|------|-------------|-------------|
| `sn-bind-data` | `CaixaData` | Binds a date value rendered as text. |
...
`CaixaData` → `BaseCaixaTexto` → `BaseControleFormulario` → `ControleRotulo` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **16** usages across **8** `.shtml` files in the Sigi frontend.
