# `<sn-caixa-cartao-credito>` — Credit card box

**Type:** Form control (`sn-`) | **Class:** `CaixaCartaoCredito` | **Extends:** `BaseCaixaTexto` | **Rendered as:** `HTMLDivElement`

Credit-card number input with brand detection and formatting.
...
|-----------|------|-------------|-------------|
| `sn-mascara` | `String` | `CaixaCartaoCredito` | Input mask (e.g. `(00) 0000-0000`). Masked input keeps the raw value in the bind. |
...
|------|-------------|-------------|
| `sn-bind-texto` | `CaixaCartaoCredito` | One-way bind that renders the value as the element text (also `{{path}}` in content). |
...
`CaixaCartaoCredito` → `BaseCaixaTexto` → `BaseControleFormulario` → `ControleRotulo` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
