# `<sn-item-selecionado-template>` — Selected-item template (alias)

**Type:** Form control (`sn-`) | **Class:** `ItemTemplateSelecionado` | **Extends:** `BaseTemplate` | **Rendered as:** `HTMLDivElement`

> Also registered as: `<sn-item-template-selecionado>`

Alias of `sn-item-template-selecionado`.

## Real-world examples

Extracted from the Sigi frontend (16 usages in 13 files).
...
```html
<sn-item-selecionado-template ap-altura-linha="25px">
...
```html
<sn-item-selecionado-template ap-altura-linha="28px">
        <span sn-bind-texto="{{Formato}}"></span>
        (<span sn-bind-texto="{{Extensoes}}"></span>)
    </sn-item-selecionado-template>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Encadernacao/Compartilhado/TipoGravacao/JanelaCadastroTipoGravacaoArquivo.shtml`*
...
Found **16** usages across **13** `.shtml` files in the Sigi frontend.
