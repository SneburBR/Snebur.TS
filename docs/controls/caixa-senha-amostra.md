# `<sn-caixa-senha-amostra>` — Simple password box

**Type:** Form control (`sn-`) | **Class:** `CaixaSenhaSimples` | **Extends:** `BaseCaixaTexto` | **Rendered as:** `HTMLDivElement`

Reduced password input variant (no strength/extras).
...
|-----------|------|-------------|-------------|
| `sn-is-senha-md5` | `Boolean` | `CaixaSenhaSimples` | Hashes the typed password with MD5 before binding (legacy authentication flows). |
...
|------|-------------|-------------|
| `sn-bind-senha` | `CaixaSenhaSimples` | Binds a password value. |
| `sn-bind-texto` | `CaixaSenhaSimples` | One-way bind that renders the value as the element text (also `{{path}}` in content). |
...
`CaixaSenhaSimples` → `BaseCaixaTexto` → `BaseControleFormulario` → `ControleRotulo` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
