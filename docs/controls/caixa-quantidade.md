# `<sn-caixa-quantidade>` — Quantity box

**Type:** Form control (`sn-`) | **Class:** `CaixaQuantidade` | **Extends:** `BaseCaixaNumero` | **Rendered as:** `HTMLDivElement`

Numeric stepper for quantities (+/- buttons, min/max limits).

## Real-world examples

Extracted from the Sigi frontend (15 usages in 10 files).

```html
<sn-caixa-quantidade
        sn-nome="TxtQuantidade"
        id="TxtQuantidade"
        ap-largura="100%"
        sn-bind="{{Quantidade}}"
        sn-minimo="1"
...
```html
<sn-caixa-quantidade
        sn-visibilidade="{{IsSelecionadoTemporario}}"
        sn-minimo="0"
        sn-maximo="9999"
        ap-peso-fonte="Negrito"
        ap-margem="0px 6px 0px 0px"
        sn-bind="{{Quantidade}}"
        sn-bind-class="sn-cor-texto--falha={{IsQuantidadeInvalida}}"
        sn-passo="1"
        sn-is-rotulo-vazio="true"
        ap-largura="100px">
    </sn-caixa-quantidade>
...
*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Promocao/Cadastro/Itens/JanelaSelecionarTamanhoRevelacaoPromocao.shtml`*
...
<sn-caixa-quantidade
        ...
        sn-is-rotulo-flutuante="false"
        sn-is-rotulo-vazio="true"
        sn-tipo-entrada="Nenhum">
    </sn-caixa-quantidade>
```

*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/FotoProduto/janelapreviewfotoproduto.shtml`*
...
`CaixaQuantidade` → `BaseCaixaNumero` → `BaseCaixaTexto` → `BaseControleFormulario` → `ControleRotulo` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **15** usages across **10** `.shtml` files in the Sigi frontend.
