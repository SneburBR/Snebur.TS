# `<sn-coluna-personalizada>` — Custom column

**Type:** Form control (`sn-`) | **Class:** `TemplateColunaPersonalizada` | **Extends:** `TemplateColuna` | **Rendered as:** `HTMLDivElement`

Grid column with free-form template content (buttons, images, …).

## Real-world examples

Extracted from the Sigi frontend (535 usages in 176 files).

```html
<sn-coluna-personalizada
        sn-rotulo="Tipo do promoção"
        class="sn-esquerda"
        ap-largura="170px"
        sn-visibilidade="{{IsTipoCupomSelecionado}}, Origem=this">

        <ap-texto
            sn-bind-enum="{{Promocao.TipoPromocaoEnum}}"
            sn-tipo="sigi.EnumTipoPromocao">
        </ap-texto>
    </sn-coluna-personalizada>
```
...
```html
<sn-coluna-personalizada
        sn-rotulo="{{RotuloValor}}, Origem=this"
        class="sn-direita"
        ap-largura="120px"
        sn-visibilidade="{{IsMostrarValor}}, Origem=this">

        <ap-texto
            sn-visibilidade="{{Promocao.IsVendaOnline}}, Padrao=false"
            sn-bind="{{Promocao.Valor}}"
            sn-formatar="Moeda"
            sn-bind-class="sn-configuracao-deletada={{IsDeletado}}">
        </ap-texto>
    </sn-coluna-personalizada>
```
...
<sn-coluna-personalizada
        ap-largura="220px"
        sn-bind="{{ValorTotal}}"
        sn-rotulo="Valor total individual"
        class="sn-direita">

        <ap-texto
            sn-bind="{{ValorTotal}}"
            sn-bind-class="sn-negrito={{IsNegrito}}"
            sn-formatar="Moeda">
        </ap-texto>
    </sn-coluna-personalizada>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Pedidos/Controles/ControleResumoPedido.shtml`*
...
| `sn-rotulo` | `String` | `TemplateColuna` *(inherited)* | Label text of the control (floating label on inputs). |
...
|------|-------------|-------------|
| `sn-bind` | `TemplateColuna` *(inherited)* | Two-way bind of the control **value** to a property path of the data source (e.g. `Cliente.Nome`). Validation rules of the bound domain property are applied automatically. |
...
| `sn-bind-texto` | `TemplateColuna` *(inherited)* | One-way bind that renders the value as the element text (also `{{path}}` in content). |
...
`TemplateColunaPersonalizada` → `TemplateColuna` → `BaseTemplate` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **535** usages across **176** `.shtml` files in the Sigi frontend.
