# `<sn-bloco-template>` — Block item template

**Type:** Form control (`sn-`) | **Class:** `BlocoTemplate` | **Extends:** `BaseBlocoTemplate` | **Rendered as:** `HTMLDivElement`

Item template used by `sn-painel-lista` (block/panel-shaped items).

## Real-world examples

Extracted from the Sigi frontend (291 usages in 176 files).

```html
<sn-bloco-template
    ap-largura="25%"
    ap-largura--notebook="33.33%"
    ap-largura--tablet="50%"
    ap-largura--celular="100%"
    ap-altura="380px"
    ap-altura--notebook="350px"
    ap-altura--tablet="370px"
    ap-altura--celular="420px"
    ap-margem="0px 5px 0px 0px">
    ...
</sn-bloco-template>
```

*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Promocao/paginapromocao.shtml`*

```html
<sn-bloco-template
        ap-largura="25%"
        ap-altura="70px"
         sn-bind-class="is-selecionado={{IsSelecionado}}"
        sn-bind-estilo="background-color={{CorRgba}}"
         class="sn-item-cor-fundo" 
        sn-click="BtnSelecionar_Click" >
    </sn-bloco-template>
```

*Source: `src/frontend/Zyoncore.Sigi.FotoAlbum.TS/Apresentacao/Configuracao/ConfiguracaoDiagramador/JanelaConfiguracaoPadraoFundo.shtml`*
...
| `sn-tipo` | `String` | `BaseBlocoTemplate` *(inherited)* | Domain type name used by the control (e.g. the enum type of a `sn-combobox-enum` or entity type of a query). |
...
**`sn-click="BtnSelecionarModulo_Click"`** — `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Pedidos/PaginaPedidosPainel.shtml.ts`:

```ts
public BtnSelecionarModulo_Click(blocoTemplate: ui.BlocoTemplate, e: ui.UIEventArgs)
{
    const grupoVM = blocoTemplate.DataSource;
    if (grupoVM instanceof GrupoPedidosModuloViewModel)
    {
        this.GrupoPedidosModuloViewModelSelecionado = grupoVM;
    }
}
```
...
`BlocoTemplate` → `BaseBlocoTemplate` → `BaseTemplate` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **291** usages across **176** `.shtml` files in the Sigi frontend.
