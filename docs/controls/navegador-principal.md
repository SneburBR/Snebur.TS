# `<sn-navegador-principal>` — Main navigator

**Type:** Form control (`sn-`) | **Class:** `NavegadorPrincipal` | **Extends:** `BaseNavegador` | **Rendered as:** `HTMLDivElement`
...
Extracted from the Sigi frontend (3 usages in 3 files).

```html
<sn-navegador-principal
        sn-nome="Navegador"
        sn-pagina-inicial="PaginaPainel"
        ap-margem="24px 4px 8px 0px">
    </sn-navegador-principal>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/DocumentoPrincipal.shtml`*
...
```html
<sn-navegador-principal sn-pagina-inicial="PaginaInicial"

    ap-margem="16px"></sn-navegador-principal>
```

*Source: `src/frontend/Zyoncore.Sigi.ZyonAdmin/Apresentacao/DocumentoPrincipal.shtml`*
...
|-----------|------|-------------|-------------|
| `sn-identificador-navegador` | `String` | `NavegadorPrincipal` | Name of the target navigator (for buttons/links that navigate). |
...
`NavegadorPrincipal` → `BaseNavegador` → `BaseControleApresentacaoFormulario` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **3** usages across **3** `.shtml` files in the Sigi frontend.
