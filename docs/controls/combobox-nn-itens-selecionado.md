# `<sn-combobox-nn-itens-selecionado>` — Combobox N:N selected items

**Type:** Form control (`sn-`) | **Class:** `ComboBoxNnItensSelecionado` | **Extends:** `BaseControle` | **Rendered as:** `HTMLDivElement`

Area of `sn-combobox-nn` rendering the chips/list of selected items.
...
`ComboBoxNnItensSelecionado` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
