# `<sn-caixa-numero>` — Number box

**Type:** Form control (`sn-`) | **Class:** `CaixaNumero` | **Extends:** `BaseCaixaNumero` | **Rendered as:** `HTMLDivElement`

Numeric input with min/max (`sn-minimo`/`sn-maximo`) and integer formatting options.

## Real-world examples

Extracted from the Sigi frontend (80 usages in 35 files).

```html
<sn-caixa-numero
        class="sn-input-direita"
        sn-rotulo="Desconto"
        ap-tipo-caixa="Delineada"
        ap-largura="100%"
        sn-passo="0.01"
        sn-minimo="1"
        sn-is-formatar-inteiro="false"
        sn-maximo="99"
        sn-bind="{{Desconto}}"
        sn-tipo-entrada="Decimal"
        sn-formatar="Porcentagem1"
        sn-is-rotulo-vazio="true"
        ap-icone="Percent"
        sn-marca-dagua="Digite aqui seu desconto, somente numeros">
    </sn-caixa-numero>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Promocao/Cadastro/Wizard/PaginaWizardPromocaoPeriodoDesconto.shtml`*

```html
<sn-caixa-numero
        class="sn-input-direita"
        sn-rotulo="Prazo para utilização do cupom"
        ap-tipo-caixa="Delineada"
        ap-largura="100%"
        sn-passo="0.01"
        sn-minimo="1"
        sn-is-formatar-inteiro="false"
        sn-maximo="99"
        sn-bind="{{Promocao.PrazoUtilizacao}}"
        sn-tipo-entrada="Decimal"
        sn-is-rotulo-vazio="true"
        sn-marca-dagua="Digite aqui o prazo para utilização do cupom">
    </sn-caixa-numero>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Promocao/Cadastro/Wizard/PaginaWizardPromocaoPeriodoValePresente.shtml`*
...
*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Promocao/Cadastro/Wizard/PaginaWizardPromocaoPeriodoPacote.shtml`*
...
| `max` | `String` | `BaseCaixaNumero` *(inherited)* | Native HTML `max` attribute (managed by the framework). |
| `min` | `String` | `BaseCaixaNumero` *(inherited)* | Native HTML `min` attribute (managed by the framework). |
...
| `sn-is-formatar-inteiro` | `Number` | `BaseCaixaNumero` *(inherited)* | Formats the value as integer (no decimals). |
...
| `sn-maximo` | `Number` | `BaseCaixaNumero` *(inherited)* | Maximum numeric value accepted. |
| `sn-maxlength` | `Number` | `BaseCaixaTexto` *(inherited)* | Maximum number of characters accepted. |
| `sn-minimo` | `Number` | `BaseCaixaNumero` *(inherited)* | Minimum numeric value accepted. |
...
| `sn-passo` | `String` | `BaseCaixaNumero` *(inherited)* | Step increment of sliders/steppers. |
| `sn-passo-largo` | `String` | `BaseCaixaNumero` *(inherited)* | Large step increment (Page Up/Down or fast buttons). |
...
| `step` | `String` | `BaseCaixaNumero` *(inherited)* | Native HTML `step` attribute (managed by the framework). |
...
| `sn-bind-numero` | `BaseCaixaNumero` *(inherited)* | Binds a numeric value rendered as text. |
...
`CaixaNumero` → `BaseCaixaNumero` → `BaseCaixaTexto` → `BaseControleFormulario` → `ControleRotulo` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **80** usages across **35** `.shtml` files in the Sigi frontend.
