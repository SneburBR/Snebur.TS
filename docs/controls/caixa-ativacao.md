# `<sn-caixa-ativacao>` — Switch (activation)

**Type:** Form control (`sn-`) | **Class:** `CaixaAtivacao` | **Extends:** `BaseControleFormulario` | **Rendered as:** `HTMLDivElement`
...
|-----------|------|-------------|-------------|
| `sn-funcao-is-pode-ativar` | `String` | `CaixaAtivacao` | — |
...
|------|-------------|-------------|
| `sn-bind-texto` | `CaixaAtivacao` | One-way bind that renders the value as the element text (also `{{path}}` in content). |
...
`CaixaAtivacao` → `BaseControleFormulario` → `ControleRotulo` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
