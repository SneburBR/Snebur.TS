# `<sn-coluna-titulo>` — Column title

**Type:** Form control (`sn-`) | **Class:** `TemplateTiluloColuna` | **Extends:** `BaseTemplate` | **Rendered as:** `HTMLDivElement`

Header cell template of a grid column.
...
`TemplateTiluloColuna` → `BaseTemplate` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
