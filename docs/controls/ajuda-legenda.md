# `<sn-ajuda-legenda>` — Help caption

**Type:** Form control (`sn-`) | **Class:** `AjudaLegenda` | **Extends:** `BaseControle` | **Rendered as:** `HTMLDivElement`
...
`AjudaLegenda` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
