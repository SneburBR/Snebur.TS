# `<sn-caixa-area-texto-sugestao>` — Textarea with suggestions

**Type:** Form control (`sn-`) | **Class:** `CaixaAreaTextoSugestao` | **Extends:** `BaseCaixaTexto` | **Rendered as:** `HTMLDivElement`

Multi-line variant of `sn-caixa-texto-sugestao`.
...
|-----------|------|-------------|-------------|
| `sn-is-mostrar-contador-caracteres` | `Boolean` | `CaixaAreaTextoSugestao` | Shows the “n/max” character counter under the box. |
| `sn-nome-propriedade-sugestao` | `String` | `CaixaAreaTextoSugestao` | Property of the suggestion objects to display/complete with. |
...
|------|-------------|-------------|
| `sn-bind-texto` | `CaixaAreaTextoSugestao` | One-way bind that renders the value as the element text (also `{{path}}` in content). |
...
`CaixaAreaTextoSugestao` → `BaseCaixaTexto` → `BaseControleFormulario` → `ControleRotulo` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
