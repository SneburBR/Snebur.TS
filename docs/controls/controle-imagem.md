# `<sn-controle-imagem>` — Image control

**Type:** Form control (`sn-`) | **Class:** `ControleImagem` | **Extends:** `BaseControleImagem` | **Rendered as:** `HTMLDivElement`
...
Extracted from the Sigi frontend (38 usages in 31 files).

```html
<sn-controle-imagem ap-largura="100%" ap-altura="186px"
        ap-cor-fundo="Cinza" ap-tonalidade-fundo="T300"
        sn-bind-imagem="{{CorRevestimento.Imagem}}"
        sn-tamanho-imagem="Pequena"
        sn-preenchimento-imagem="UniformeFora">
    </sn-controle-imagem>
```

*Source: `src/frontend/Zyoncore.Sigi.FotoAlbum.TS/Apresentacao/Montagem/FerramentasFlutuante/ControleFerramentaRevestimentoEstojo.shtml`*
...
<sn-controle-imagem
        ...
        sn-url-imagem-pendente="vazia"
        sn-bind="{{ImagemMockupEstojo}}">
    </sn-controle-imagem>
```

*Source: `src/frontend/Zyoncore.Sigi.FotoAlbum.TS/Apresentacao/Montagem/Diagramador/DiagramadorAlbum/Mockup/JanelaMockupAlbum.shtml`*

```html
<sn-controle-imagem ap-largura="300px" ap-alinhamento-horizontal="Centro"
        sn-preenchimento-imagem="UniformeFora"
        ap-altura="300px"
        sn-url-imagem-pendente="vazia" 
        sn-bind="{{Entidade.ImagemRevestimento}}">
    </sn-controle-imagem>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Encadernacao/Compartilhado/Revestimento/JanelaCadastroCorRevestimento.shtml`*
...
`ControleImagem` → `BaseControleImagem` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **38** usages across **31** `.shtml` files in the Sigi frontend.
