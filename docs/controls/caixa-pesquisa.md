# `<sn-caixa-pesquisa>` — Search box

**Type:** Form control (`sn-`) | **Class:** `CaixaPesquisa` | **Extends:** `BaseControle` | **Rendered as:** `HTMLDivElement`

Text input specialized for filtering/searching lists — fires `sn-texto-pesquisa` as the user types (debounced).

## Real-world examples

Extracted from the Sigi frontend (7 usages in 7 files).

```html
<sn-caixa-pesquisa sn-texto-pesquisa="BtnPesquisa_Pesquisa" sn-rotulo="Pesquisa"></sn-caixa-pesquisa>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Seguranca/Cadastro/JanelaBuscarUsuario.shtml`*

```html
<sn-caixa-pesquisa ap-alinhamento-horizontal="Esquerda"></sn-caixa-pesquisa>
```

*Source: `src/frontend/Zyoncore.Sigi.ZyonAdmin/Apresentacao/Conta/PaginaDominiosConta.shtml`*

```html
<sn-caixa-pesquisa></sn-caixa-pesquisa>
```

*Source: `src/frontend/Zyoncore.Sigi.ZyonAdmin/Apresentacao/Cadastros/TemplatesAlbum/PaginaTemplatesAlbum.shtml`*
...
|-------|--------|------------|
| `sn-texto-pesquisa` | `CaixaPesquisa` | Fires (debounced) when the search text changes. |
...
`CaixaPesquisa` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **7** usages across **7** `.shtml` files in the Sigi frontend.
