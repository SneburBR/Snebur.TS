# `<sn-exibir-codigo>` — Code viewer

**Type:** Form control (`sn-`) | **Class:** `ExibirCodigo` | **Extends:** `BaseControle` | **Rendered as:** `HTMLDivElement`

Renders a block of code/markup for display (used by internal demo/manual pages).
...
`ExibirCodigo` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
