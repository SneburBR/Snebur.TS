# `<sn-data-lista>` — Data grid

**Type:** Form control (`sn-`) | **Class:** `DataLista` | **Extends:** `BaseControleLista` | **Rendered as:** `HTMLDivElement`
...
Extracted from the Sigi frontend (146 usages in 135 files).

```html
<sn-data-lista sn-bind="{{PedidosViewModel}}, Origem=this"
    sn-linha-detalhes-expandida="DataListaPedidos_LinhaDetalhesExpandia"
    ap-margem="16px 8px"
    ap-margem--celular="0px 8px">
    ...
</sn-data-lista>
```

*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Pedido/paginapedidos.shtml`*

```html
<sn-data-lista sn-nome="DataListaPrecosEstojo" ap-altura-linha="32px" sn-bind-lista="{{GruposTipoEstojoViewModel}}"
    sn-linha-detalhes-expandida="DataLista_LinhaDetalhesExpandida" ap-margem="0px 16px">
    ...
</sn-data-lista>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Cobranca/TabelaPreco/Estojo/PaginaPrecosEstojo.shtml`*

```html
<sn-data-lista sn-nome="DataLista" ap-altura-linha="32px" sn-bind-lista="{{PrecosGrupoCapaViewModel}}" sn-linha-detalhes-expandida="DataLista_LinhaDetalhesExpandida" ap-margem="0px 0px 10px 0px" >
    ...
</sn-data-lista>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Cobranca/TabelaPreco/Album/Avancado/Capa/PaginaPrecosCapaAvancado.shtml`*
...
|-----------|------|-------------|-------------|
| `sn-desativar-efeito-hover` | `Boolean` | `DataLista` | Disables the row hover effect. |
| `sn-is-marcar-item` | `Boolean` | `DataLista` | Debug: highlights the item under the cursor. |
| `sn-is-marcar-linha` | `Boolean` | `DataLista` | Debug: highlights the row under the cursor. |
...
|-------|--------|------------|
| `sn-linha-click` | `DataLista` | Fires when a grid row is clicked. |
| `sn-linha-detalhes-expandida` | *standard event* | Fires when a grid row detail area is expanded. |
| `sn-ordenacao-coluna-alterada` | *standard event* | Fires when the grid sort column/direction changes. |
...
**`sn-linha-detalhes-expandida="DataLista_LinhaDetalhesExpandida"`** — `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Cobranca/TabelaPreco/Estojo/PaginaPrecosEstojoAvancado.shtml.ts`:

```ts
private async DataLista_LinhaDetalhesExpandida(provedor: ui.BaseUIElemento, e: ui.LinhaDetalhesExpandidaEventArgs)
{
    const precoGrupoEstojo = provedor.DataSource;
    if (precoGrupoEstojo instanceof GrupoTipoEstojoEspessuraViewModel)
    {
        if (!precoGrupoEstojo.IsCarregado)
        {

            const controleDetalhes = e.LinhaDetalhes.RetornarControleFilho(ControleDetalhesPrecoEstojo, true);
            await controleDetalhes.CarregarAsync(precoGrupoEstojo);
            precoGrupoEstojo.IsCarregado = true;
        }
    }
}
```

**`sn-ordenacao-coluna-alterada="DataLista_OrdenacaoAlterada"`** — `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Clientes/PaginaClientes.shtml.ts`:

```ts
public DataLista_OrdenacaoAlterada(dataLista: ui.DataLista, e: ui.OrdenacaoColunaAlteradaEventArgs)
{
    if (e.Parametros?.ContainsKey("Ordenacao"))
    {
        this.Paginacao.Resetar();

        const ordenacao = e.Parametros.Item("Ordenacao");
        const sentido = e.Coluna.SentidoOrdenacao;
        this.Ordenacao = this.RetornarOrdenacao(ordenacao, sentido);
        this.Sentido = this.RetornarSentido(sentido);
...
`DataLista` → `BaseControleLista` → `BaseControleApresentacaoFormulario` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **146** usages across **135** `.shtml` files in the Sigi frontend.
