# `<sn-rodape>` — List footer

**Type:** Form control (`sn-`) | **Class:** `Rodape` | **Extends:** `BaseControle` | **Rendered as:** `HTMLDivElement`

Footer area of a list control (below the items; totals, actions).
...
`Rodape` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
