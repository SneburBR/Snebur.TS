# `<sn-cabecalho>` — List header

**Type:** Form control (`sn-`) | **Class:** `Cabecalho` | **Extends:** `BaseControle` | **Rendered as:** `HTMLDivElement`

Header area of a list control (title row above the items).

## Real-world examples

Extracted from the Sigi frontend (1 usages in 1 files).

```html
<sn-cabecalho>

        <ap-painel
            ap-tipo-painel="PilhaHorizontalCheia" ap-tipografia="SubTitulo">
            <ap-bloco>
                <ap-texto sn-bind="{{DescricaoTipoProduto}}, Origem=this"></ap-texto>
            </ap-bloco>
            <ap-bloco ap-margem-interna="16px 0px 0px 0px">
                Grupo
            </ap-bloco>
            <ap-bloco ap-largura="280px">

                <ap-texto ap-alinhamento-horizontal="Direita">Preço a partir de:</ap-texto>
            </ap-bloco>
...
*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Cobranca/TabelaPreco/FotoProduto/PaginaTabelasPrecoFotoProdutoConfiguracao.shtml`*
...
|-------|--------|------------|
| `sn-texto-pesquisa` | `Cabecalho` | Fires (debounced) when the search text changes. |
...
`Cabecalho` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **1** usages across **1** `.shtml` files in the Sigi frontend.
