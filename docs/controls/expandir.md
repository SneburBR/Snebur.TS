# `<sn-expandir>` — Expander

**Type:** Form control (`sn-`) | **Class:** `Expandir` | **Extends:** `BaseControle` | **Rendered as:** `HTMLDivElement`
...
|------|-------------|-------------|
| `sn-bind-data-source` | `Expandir` | Sets the data-source object for this subtree (entity, view-model or query result). |
...
`Expandir` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
