# `<sn-combobox-item-selecionado>` — Combobox selected-item area

**Type:** Form control (`sn-`) | **Class:** `ComboBoxItemSelecionado` | **Extends:** `BaseControle` | **Rendered as:** `HTMLDivElement`
...
`ComboBoxItemSelecionado` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
