# `<sn-combobox-enum>` — Combobox (enum)

**Type:** Form control (`sn-`) | **Class:** `ComboBoxEnum` | **Extends:** `ComboBox` | **Rendered as:** `HTMLDivElement`

Dropdown over the values of a domain enum. `sn-tipo` names the enum type; the numeric value binds with `sn-bind`.

## Real-world examples

Extracted from the Sigi frontend (50 usages in 35 files).

```html
<sn-combobox-enum
        sn-rotulo="Selecione identidade de gênero "
        sn-rotulo-flutuante="Identidade de gênero "
        sn-bind="{{Genero}}"
        ap-largura="100%"
        sn-tipo="sigi.EnumGenero"
        ap-margem="0px 2px 0px 0px"
        sn-is-rotulo-vazio="false"
        sn-is-rotulo-flutuante="true"
        sn-permitir-limpar="false" >
    </sn-combobox-enum>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Clientes/Controles/ControleCadastroClienteEstrangeiro.shtml`*

```html
<sn-combobox-enum
        sn-rotulo="Posições dos quadros"
        ap-largura="100%"
        sn-bind="{{Entidade.PosicaoQuadrosFotoCapa}}"
        sn-bind-lista="{{PosicoesQuadros}}, Origem=this"
        sn-tipo="fotoAlbumE.EnumPosicaoQuadrosFotoCapa"
        sn-visibilidade="{{Entidade.IsPossuiQuadroFoto}}"
        sn-desabilitar="{{IsEditando}}, Origem=this"
        sn-permitir-limpar="false">
        <!--sn-item-selecionado-alterado="CmbPosicaoQuadrios_ItemSelecionadoAlterado"-->
    </sn-combobox-enum>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Encadernacao/Album/TipoCapa/JanelaCadastroTipoCapaRevestida.shtml`*

```html
<sn-combobox-enum
        sn-rotulo="Local dos quadros"
        ap-largura="100%"
        sn-bind="{{Entidade.PosicaoQuadrosFotoCapa}}"
        ap-margem="0px 4px"
        sn-bind-lista="{{PosicoesQuadros}}, Origem=this"
...
*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Encadernacao/Album/TipoCapa/JanelaCadastroTipoCapaMultiplosRevestimentos.shtml`*
...
|-----------|------|-------------|-------------|
| `sn-tipo` | `String` | `ComboBoxEnum` | Domain type name used by the control (e.g. the enum type of a `sn-combobox-enum` or entity type of a query). |
...
| `sn-is-rotulo-flutuante` | `Boolean` | `ComboBox` *(inherited)* | Enables/disables the floating-label behaviour. |
...
| `sn-permitir-limpar` | `Boolean` | `ComboBox` *(inherited)* | Shows a clear (×) affordance to unset the selection. |
| `sn-rotulo` | `String` | `BaseControleFormulario` *(inherited)* | Label text of the control (floating label on inputs). |
| `sn-rotulo-flutuante` | `String` | `ComboBox` *(inherited)* | Text of the floating label (when different from `sn-rotulo`). |
...
**`sn-valor-alterado="CmbTIpoFace_ValorAlterado"`** — `src/frontend/Zyoncore.Sigi.ZyonAdmin/Apresentacao/Cadastros/TemplatesAlbum/JanelaCadastroTemplateAlbum.shtml.ts`:

```ts
private CmbTIpoFace_ValorAlterado(provedor: ui.BaseUIElemento, e: ui.UIValorAlteradoEventArgs)
{
    const tipoFaceSelecionado = e.Valor as EnumTipoFace;

    switch (tipoFaceSelecionado)
    {
        case EnumTipoFace.Lamina:
            this.TipoFaceQuantidade = "Número de lâminas";
            return;
        case EnumTipoFace.Pagina:
            this.TipoFaceQuantidade = "Número de páginas";
            return;
        default:
            throw new Erro("tipo face não suportada");
    }
...
`ComboBoxEnum` → `ComboBox` → `BaseComboBox` → `BaseControleFormulario` → `ControleRotulo` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **50** usages across **35** `.shtml` files in the Sigi frontend.
