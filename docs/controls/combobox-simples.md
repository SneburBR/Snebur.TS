# `<sn-combobox-simples>` — Simple combobox

**Type:** Form control (`sn-`) | **Class:** `ComboBoxSimples` | **Extends:** `BaseControle` | **Rendered as:** `HTMLDivElement`

Lightweight dropdown over simple values/options (no entity query machinery).
...
|-----------|------|-------------|-------------|
| `sn-exibicao` | `String` | `ComboBoxSimples` | Display expression/format for the item text. |
| `sn-tipo` | `String` | `ComboBoxSimples` | Domain type name used by the control (e.g. the enum type of a `sn-combobox-enum` or entity type of a query). |
...
`ComboBoxSimples` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
