# `<sn-menu-item>` — Menu item

**Type:** Form control (`sn-`) | **Class:** `MenuItem` | **Extends:** `BaseMenu` | **Rendered as:** `HTMLDivElement`

Item of a `sn-menu`: icon, label and either `sn-click` or `sn-link-rota` navigation.

## Real-world examples

Extracted from the Sigi frontend (103 usages in 2 files).

```html
<sn-menu-item sn-rotulo="Clientes"
                  ap-icone="Groups"
                  sn-parametros="IsPaginaBase=true"
                  sn-tags="pedido, pedidos"
                  ap-icone-categoria="Outlined"
                  sn-navegar="PaginaClientes"
                  sigi-permissao="PermissaoConfiguracoes.Clientes">
    </sn-menu-item>
```
...
```html
<sn-menu-item sn-rotulo="Painel"
                  ap-icone="Dashboard"
                  sn-navegar="PaginaPainel"
                  ap-icone-categoria="Outlined"
                  sn-parametros="IsPaginaBase=true"
                  sn-tags="pedido, pedidos">
    </sn-menu-item>
...
```html
<sn-menu-item sn-rotulo="Pedidos"
                  ap-icone="EventNote"
                  sn-navegar="PaginaPedidos"
                  sn-parametros="IsPaginaBase=true"
                  sn-tags="pedido, pedidos"
                  sigi-permissao="PermissaoPedidos.VisualizarPedidos">
    </sn-menu-item>
```
...
`MenuItem` → `BaseMenu` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **103** usages across **2** `.shtml` files in the Sigi frontend.
