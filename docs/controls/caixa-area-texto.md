# `<sn-caixa-area-texto>` — Multi-line text box

**Type:** Form control (`sn-`) | **Class:** `CaixaAreaTexto` | **Extends:** `BaseControleFormulario` | **Rendered as:** `HTMLDivElement`

Textarea variant of `sn-caixa-texto` for long text.

## Real-world examples

Extracted from the Sigi frontend (19 usages in 18 files).

```html
<sn-caixa-area-texto
        sn-nome="TxtObservacao"
        sn-bind="{{ObservacaoPedido}}"
        sn-visibilidade="{{IsMostrarObservacao === true}}"
        sn-rotulo="Observação do pedido"
        ap-largura="100%"
        ap-altura="100px"
        ap-alinhamento-vertical="Centro"
        rows="2"
        sn-maxlength="300"
        sn-is-mostrar-contador-caracteres="true">
    </sn-caixa-area-texto>
```

*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Carrinho/Pagamento/PaginaPagamento.shtml`*

```html
<sn-caixa-area-texto
        sn-nome="TxtCaixaAreaTexto"
        ap-largura="100%"
        ap-altura="280px"
        sn-is-rotulo-vazio="true"
        ap-tipo-caixa="Delineada"
        sn-marca-dagua=""
        sn-bind-class="sn-codigo-automatico={{IsGerarCodigoAutomaticamente}}"
        class="sn-codigo-promocao"
        style="text-transform: uppercase;"
        sn-somente-leitura="{{IsGerarCodigoAutomaticamente}}">
    </sn-caixa-area-texto>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Promocao/Cadastro/Compartilhado/ControleCodigosPromocao.shtml`*
...
<sn-caixa-area-texto
        ...
        sn-maxlength="500"
        sn-is-mostrar-contador-caracteres="true"
        sn-bind="{{Entidade.DescricaoDetalhada}}"
        sn-marca-dagua="Digite aqui a descrição detalha da promoção (opcional)">
    </sn-caixa-area-texto>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Promocao/Cadastro/Wizard/PaginaWizardPromocaoTipoCupom.shtml`*
...
|-----------|------|-------------|-------------|
| `sn-is-mostrar-contador-caracteres` | `Boolean` | `CaixaAreaTexto` | Shows the “n/max” character counter under the box. |
| `sn-is-rotulo-flutuante` | `Boolean` | `CaixaAreaTexto` | Enables/disables the floating-label behaviour. |
| `sn-is-rotulo-vazio` | `Boolean` | `CaixaAreaTexto` | Reserves the label space even without label text (keeps forms aligned). |
| `sn-marca-dagua` | `String` | `CaixaAreaTexto` | Placeholder (watermark) text shown while the box is empty. Exclusive to text-box family controls (`BaseCaixaTexto`). |
| `sn-maxlength` | `Number` | `CaixaAreaTexto` | Maximum number of characters accepted. |
...
|------|-------------|-------------|
| `sn-bind-texto` | `CaixaAreaTexto` | One-way bind that renders the value as the element text (also `{{path}}` in content). |
...
`CaixaAreaTexto` → `BaseControleFormulario` → `ControleRotulo` → `BaseControle` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **19** usages across **18** `.shtml` files in the Sigi frontend.
