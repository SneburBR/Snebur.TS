# `<ap-paragrafo>` — Paragraph

**Type:** Layout component (`ap-`) | **Class:** `Paragrafo` | **Extends:** `ComponenteApresentacaoRotulo`

Paragraph text block — same features as `ap-texto` with block layout.
...
<ap-paragrafo
        sn-nome="TextoNome"
        sn-bind-texto="{{DescricaoProjetoCompleta}}, Origem=this, Converter=QuebrarLinhasHtml"
        ap-tipografia="Descricao"
        ap-quebrar="Quebrar"
        ap-margem="0px 0px 4px 0px">
    </ap-paragrafo>
```

*Source: `src/frontend/Zyoncore.Sigi.FotoAlbum.TS/Apresentacao/Montagem/Diagramador/DiagramadorAlbum/Usuario/ControleInformacaoAlbum.shtml`*

```html
<ap-paragrafo sn-nome="ConteudoTermoUso">
    </ap-paragrafo>
```

*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Soliticacao/Login/JanelaTermoUso.shtml`*

```html
<ap-paragrafo ap-margem="0px 20px 0px 4px">
        A linha pontilhada indica o recorte da imagem.  
    </ap-paragrafo>
```

*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Revelacao/janelaautorizorecorte.shtml`*
...
`Paragrafo` → `ComponenteApresentacaoRotulo` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
