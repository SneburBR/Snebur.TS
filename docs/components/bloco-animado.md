# `<ap-bloco-animado>` — Animated block

**Type:** Layout component (`ap-`) | **Class:** `BlocoAnimado` | **Extends:** `BasePainel`

Block with enter/exit animation support.
...
```html
<ap-bloco-animado
    sn-nome="BlocoInfoEntrecoRetirada"
    class="info-loja-animado"
...
*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Carrinho/Entrega/PaginaEntrega.shtml`*
...
*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Cabecalho/ControleMenuGaveta.shtml`*
...
`BlocoAnimado` → `BasePainel` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
