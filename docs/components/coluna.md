...
**Type:** Layout component (`ap-`) | **Class:** `ColunaPainel` | **Extends:** `BaseColuna`

Column region inside `ap-painel-horizontal`. Give it `ap-largura` (e.g. 50%).

## Real-world examples

Extracted from the Sigi frontend (318 usages in 86 files).

```html
<ap-coluna
    sn-nome="BlocoAcoes"
    ap-largura="90px"
    ap-largura--celular="70px"
    sn-visibilidade="{{IsSelecionado}}" a
    p-cor-fundo="Vermelha">
    ...
</ap-coluna>
```

*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Revelacao/controleimagemrevelacao.shtml`*

```html
<ap-coluna
    sn-nome="ColunaDireitaImagens"
    class="coluna-imagens"
    ap-largura="160px"
    ap-largura--celular="80px"
    ap-margem-interna="0px 38px 0px 3px"
...
```html
<ap-coluna
    sn-nome="ColunaDireita"
    ap-largura="100px"
    ap-largura--tablet="70px"
    ap-visibilidade="Visivel"
    ap-visibilidade--celular="Oculto"
    ap-margem-interna="4px 0px 0px 4px"
    >
    ...
</ap-coluna>
```
...
`ColunaPainel` → `BaseColuna` → `BasePainel` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **318** usages across **86** `.shtml` files in the Sigi frontend.
