# `<ap-bloco-cabecalho>` — Block header

**Type:** Layout component (`ap-`) | **Class:** `BlocoCabecalho` | **Extends:** `BasePainel`

Header block for cards/sections (title styling).
...
`BlocoCabecalho` → `BasePainel` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`
