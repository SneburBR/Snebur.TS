...
**Type:** Layout component (`ap-`) | **Class:** `LinhaPainel` | **Extends:** `BaseLinha`

Row region inside `ap-painel-vertical`.

## Real-world examples

Extracted from the Sigi frontend (622 usages in 423 files).

```html
<ap-linha
    ap-altura="80px"
    sn-visibilidade="{{ DadosEntrega != null }}"
...
<ap-linha
    ap-altura="45px"
    ap-altura--celular="60px"
    sn-visibilidade="{{IsMostrarTiposServico}}" ap-barra-rolagem-horizontal="Automatica"  >
    ...
</ap-linha>
```

*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/TabelaPreco/JanelaTabelaPrecos.shtml`*

```html
<ap-linha ap-altura="40px" ap-altura--celular="40px"
    sn-visibilidade="{{IsFiltrarPorData}},Origem=this" ap-margem--celular="0px 10px 0px 0px">
    ...
</ap-linha>
```
...
**`sn-arrastar-arquivos="Bloco_ArrastarArquivos"`** — `src/frontend/Zyoncore.Sigi.ZyonAdmin/Apresentacao/Aplicacao/JanelaCadastroVersaoAplicacaoFotoAlbum.shtml.ts`:

```ts
private Bloco_ArrastarArquivos(provedor: any, e: ui.SelecionarArquivosEventoArgs)
{
...
`LinhaPainel` → `BaseLinha` → `BasePainel` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **622** usages across **423** `.shtml` files in the Sigi frontend.
