# `<ap-rodape>` — Footer region

**Type:** Layout component (`ap-`) | **Class:** `RodapePainel` | **Extends:** `BaseLinha`

Bottom-docked region of `ap-painel-vertical`.

## Real-world examples

Extracted from the Sigi frontend (185 usages in 167 files).

```html
<ap-rodape
    ap-altura="45px"
    ap-altura--celular="38px"
    sn-visibilidade="{{IsMostrarAcoes}}, Origem=this"
    ap-cor-fundo="Cinza"
    ap-tonalidade-fundo="Aplha10"

    class="info-configuracao">
    ...
</ap-rodape>
```
...
`RodapePainel` → `BaseLinha` → `BasePainel` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **185** usages across **167** `.shtml` files in the Sigi frontend.
