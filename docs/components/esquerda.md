# `<ap-esquerda>` — Left region

**Type:** Layout component (`ap-`) | **Class:** `EsquerdaPainel` | **Extends:** `BaseColuna`

Left-docked region of `ap-painel-horizontal`.

## Real-world examples

Extracted from the Sigi frontend (17 usages in 15 files).

```html
<ap-esquerda
        sn-nome="ColunaLocalMenuEsquerda"
        ap-largura="240px"
        ap-tonalidade-fundo="T200"
        ap-cor-texto="Cinza"
...
*Source: `src/frontend/Zyoncore.Sigi.FotoAlbum.TS/Apresentacao/Configuracao/JanelaSelecao.shtml`*

```html
<ap-esquerda
        ap-largura="40px"
        ap-altura--celular="30px">

        <ap-icone ap-icone="Upgrade"
            class="sn-texto-empresa"
            ap-tamanho-icone="Medio">
        </ap-icone>
    </ap-esquerda>
```

*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Envio/PaginaFilaEnvio.shtml`*

```html
<ap-esquerda
        ap-largura="300px"
        ap-largura--celular="Auto">
        ...
        <ap-bloco ap-altura="100%"
            ...
            <img   
                sn-click="BtnInicio_Click"
                class="sn-imagem-uniforme-vertical-centro"
                sn-bind-url-imagem="{{UrlLogo}}, Origem=this" />
...
*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Cabecalho/ControleMenuCabecalhoProfissional.shtml`*
...
`EsquerdaPainel` → `BaseColuna` → `BasePainel` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **17** usages across **15** `.shtml` files in the Sigi frontend.
