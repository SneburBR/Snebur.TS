# `<ap-estilo-itens>` — Item styles

**Type:** Layout component (`ap-`) | **Class:** `EstilosItens` | **Extends:** `ComponenteApresentacao`

Declares shared styles applied to the items of the parent list/menu (selection color, hover, spacing).

## Real-world examples

Extracted from the Sigi frontend (28 usages in 23 files).

```html
<ap-estilo-itens ap-largura="50%" ap-margem="8px"
        ap-cor-fundo="SistemaPrincipal"
        ap-margem-interna="16px" ap-cor-texto="Branca">
    </ap-estilo-itens>
```

*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Inicio/PaginaInicioProfissional.shtml`*
...
<ap-estilo-itens
        ap-margem="8px"
        ap-cor-fundo="Cinza"
        ap-tonalidade-fundo="Aplha10">
    </ap-estilo-itens>
```

*Source: `src/frontend/Zyoncore.Sigi.WebAdmin/Apresentacao/Projetos/ControleRelatorioEstojo.shtml`*
...
`EstilosItens` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **28** usages across **23** `.shtml` files in the Sigi frontend.
