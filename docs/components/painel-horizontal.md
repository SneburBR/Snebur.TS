# `<ap-painel-horizontal>` — Horizontal panel

**Type:** Layout component (`ap-`) | **Class:** `PainelHorizontal` | **Extends:** `BasePainel`

Three-region horizontal layout: `ap-esquerda` (left), `ap-coluna` (center columns) and `ap-direita` (right).

## Real-world examples

Extracted from the Sigi frontend (123 usages in 86 files).

```html
<ap-painel-horizontal
    ap-altura-minima="70px"
    ap-altura-minima--celular="50px"
    sn-click="BtnSelecionarPagamento_Click"
    ap-margem="0px 8px"
    ap-margem--celular="0px 10px">
    ...
</ap-painel-horizontal>
```
...
<ap-painel-horizontal
    ...
    ap-altura="22px"
    sn-bind-class="fotoalbum-camada-oculta={{IsOculto}}" ap-cor-texto="Cinza"
    ap-tonalidade-texto="T700">
    ...
</ap-painel-horizontal>
```

*Source: `src/frontend/Zyoncore.Sigi.FotoAlbum.TS/Apresentacao/Montagem/MenuItens/Camadas/ControleItemCamada.shtml`*

```html
<ap-painel-horizontal
    ap-altura="124px"
    class="sn-borda-item"
    ap-margem-interna="8px"
    sn-bind-class="sn-is-selecionado={{IsSelecionado}}">
    ...
</ap-painel-horizontal>
```

*Source: `src/frontend/Zyoncore.Sigi.FotoAlbum.TS/Apresentacao/Montagem/Diagramador/Encadernacao/ControlerGravacaoEncadernacao.shtml`*
...
**`sn-click="BtnSelecionarPagamento_Click"`** — `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Carrinho/Pagamento/PaginaPagamento.shtml.ts`:

```ts
private BtnSelecionarPagamento_Click(provedor: ui.BaseUIElemento, e: ui.UIEventArgs)
{
    const itemPagamentoVM = provedor.DataSource;
    if (itemPagamentoVM instanceof ItemPagamentoViewModel)
    {
        this.ViewModel.SelecionarPagamento(itemPagamentoVM);
    }
}
```
...
`PainelHorizontal` → `BasePainel` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **123** usages across **86** `.shtml` files in the Sigi frontend.
