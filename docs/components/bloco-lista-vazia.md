# `<ap-bloco-lista-vazia>` — Empty-list block

**Type:** Layout component (`ap-`) | **Class:** `BlocoListaVazia` | **Extends:** `BasePainel`

Placed inside a list control; shown automatically when the bound collection is empty (“no results” message).

## Real-world examples

Extracted from the Sigi frontend (8 usages in 8 files).
...
<ap-bloco-lista-vazia
        ...
        <ap-bloco-item
            ...
            <ap-painel ap-tipo-painel="PilhaVertical">
                <ap-texto ap-margem="0px 24px" ap-tipografia="SubTitulo">Nenhuma linha álbum cadastrada .</ap-texto>
                <ap-bloco>
                    <sn-botao
                        sn-click="BtnNovo_Click"
                        ap-cor-texto="Branca"
                        ap-cor-fundo="SistemaPrincipal"
                        ap-alinhamento-horizontal="Centro" ap-altura="80px"
                        ap-tipografia="Titulo" ap-largura="220px"
                        ap-altura-linha="60px">
                        Cadastrar agora
                    </sn-botao>
                </ap-bloco>
            </ap-painel>
...
```html
<ap-bloco-lista-vazia ap-altura="100%">
        <!--- lista vazia-->
        <ap-painel ap-tipo-painel="PilhaVertical" ap-margem="24px 16px">
            <ap-texto ap-tipografia="h5">
                Ops!
            </ap-texto>
            <ap-texto
                ap-tipografia="h5"
                ap-margem="0px 16px">
                Não existem cupons ou promoções disponíveis no momento.
            </ap-texto>
        </ap-painel>
...
<ap-bloco-lista-vazia>
        <ap-painel ap-tipo-painel="PilhaVertical"
            ...
            <ap-texto ap-tipografia="h5" ap-margem="0px 16px">
                Nenhum item foi encontrado ou está disponível no momento.
            </ap-texto>
        </ap-painel>
    </ap-bloco-lista-vazia>
```

*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/FotoProduto/BasePaginaProduto.shtml`*
...
`BlocoListaVazia` → `BasePainel` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **8** usages across **8** `.shtml` files in the Sigi frontend.
