...
**Type:** Layout component (`ap-`) | **Class:** `Icone` | **Extends:** `ComponenteApresentacaoConteudo`
...
Extracted from the Sigi frontend (190 usages in 93 files).

```html
<ap-icone ap-icone="Add"
        sn-tipo-botao="Icone"  
        sn-visibilidade="{{IsExpandido === false}}" 
        ap-margem="0px -16px 0px 0px" ap-margem--celular="0px">
    </ap-icone>
```

*Source: `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/TabelaPreco/PaginaRevelacaoDescontos.shtml`*

```html
<ap-icone ap-icone="{{Icone}}"
        ap-tamanho-icone="Medio"
        ap-tamanho-icone--celular="Padrao"
...
<ap-icone
        ap-icone="Check"
        sn-visibilidade="{{ IsPendente == false }}"
        ap-alinhamento-horizontal="Centro"
        ap-alinhamento-vertical="Centro"
        ap-cor-texto="SistemaSucesso">
    </ap-icone>
```
...
**`sn-click="BtnFechar_Click"`** — `src/frontend/Zyoncore.Sigi.Online/Apresentacao/Principal/Cabecalho/ControleMenuGaveta.shtml.ts`:
...
`Icone` → `ComponenteApresentacaoConteudo` → `ComponenteApresentacao` → `BaseUIElemento`

## Usage in Sigi

Found **190** usages across **93** `.shtml` files in the Sigi frontend.
